...
#'
#' @param object A `mira` object, or a list of fitted model objects, where the `l`th element gives the model fit to the `l`th imputed dataset.
...
#' Suitable `coef()` and `vcov()` methods must be available for the fitted models. Classes supported include outputs from `lm()`, `glm()` and `survival::coxph()` (where `dfcom` must be user-supplied for `coxph()` models).
#'
