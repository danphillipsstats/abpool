...
abpool <- function(object, parameters = NULL, dfcom = NULL, J = 1) {
  ...
  if (is.null(dfcom.vec) || any(sapply(dfcom.vec,is.null))){stop("Please supply a value of `dfcom`. Usually, an appropriate choice will be one of: Inf, for models where the variance is a deterministic function of the estimate; n - p, for sample size n and number of parameters p; or n - n_event for the Cox model, where n_event is the number of events.",
